...
const VIRTIO_MMIO_BASE: usize = 0xffff00000a004000;
...
register_structs! {
  ...
  VirtioMmioBlock {
    ...
    /// 设为0,表示对前者的前32个bit进行设置；设为1,则是后32个bit
...
/// 驱动程序对virtio_blk进行初始化，并设置virtqueue
pub fn virtio_blk_init() {
    let mmio = &VIRTIO_MMIO;
    if mmio.MagicValue.get() != 0x74726976 // 固定
        || mmio.Version.get() != 2         // 固定
        || mmio.DeviceID.get() != 2        // 表示virtio-blk
        || mmio.VendorID.get() != 0x554d4551 // qemu的ascii码
    {
...
/// device 可写，如果没有这个标志则为只读
const VRING_DESC_F_WRITE: u16 = 2;
const VRING_DESC_F_INDIRECT: u16 = 4;
/// sector：起始扇区数; count：要读取的扇区数; buf：缓冲区的物理地址
